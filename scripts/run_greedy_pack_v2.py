...
def _update_latest_manifest(public_outputs_root: Path, key: str, run_folder_name: str):
    """
    latest.json will contain multiple keys, e.g.:
      greedy_dem, greedy_rep, greedy2_dem, greedy2_rep, kmeans_softcap, ...
    """
...
def _export_frontend_files(pack_dir: Path, pack, labels, run_dir: Path, title: str):
    """
    Writes:
      - map_data.geojson (precinct features with properties + district)
      - districts.geojson (dissolved district polygons for bold outlines)
      - district_stats.json (sidebar)
      - map.png (preview)
    """
    run_dir.mkdir(parents=True, exist_ok=True)

    # GeoDataFrame of precinct shapes
    ...
    # district stats from attributes.csv (authoritative vote/weight columns)
    ...
    # merge district totals onto each precinct for tooltip
    df = df.merge(
...
def main():
    ...
    print("  maximize:", maximize, "num_districts:", num_districts, "pop_tolerance:", pop_tolerance)

    pack = load_map_pack(pack_dir)
    print("  loaded units:", len(pack.ids))

    # ----------------------------
    # V2 ALGO BLOCK (edit freely)
    # ----------------------------
    ...
    # Keep the same post-processing for now (you can modify during debugging)
