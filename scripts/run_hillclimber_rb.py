...
from gerry.viz.frame_recorder_rb import FrameRecorder, FrameMeta
...
# ---------------------------------------------------------------------
# NEW helper: compute "district changed this step"
# ---------------------------------------------------------------------
def _changed_district(prev_labels: np.ndarray | None, labels: np.ndarray) -> int | None:
    """
    Returns the district label that received at least one moved unit this step.
    If no prev_labels (first frame) or no changes, returns None.
    """
    if prev_labels is None:
        return None
    diff = np.where(prev_labels != labels)[0]
    if diff.size == 0:
        return None

    # Most moved-to district among changed units
    moved_to = labels[diff]
...
def main():
    ...
    # IMPORTANT: default to 1 so we save ALL frames
    ap.add_argument("--frame_every", type=int, default=1, help="Record a flipbook frame every N steps. Use 1 for all.")
    ...
    # NEW: color_mode="party" (red/blue) and enable highlighting
    rec = FrameRecorder(
        pack_dir=pack_dir,
        run_dir=run_dir,
        state=args.state,
    ...
    def on_frame(step: int, labels: np.ndarray, stats: dict):
        nonlocal frame_no, prev_labels

        changed_d = _changed_district(prev_labels, labels)
        ...
        rec.record(
            ...
            labels=labels,
            meta=meta,
            # precinct borders (subtle)
            edgecolor=None,
            linewidth=0.10,
            # NEW: bold outline the district that changed this step
            highlight_district=changed_d,
    ...
    labels_final = hillclimb_max_seats(
        ...
        cfg=hc_cfg,
        on_frame=on_frame,
        frame_every=FRAME_EVERY,  # default=1 => saves all frames
    )
